...
const submitReview = async (req, res) => {
  try {
    ...
    // Validate productId — must be a valid ObjectId if provided
    ...
    // Handle File Uploads to S3 — wrapped in try/catch so a failed upload
    // does NOT crash the whole review submission
    const imageURLs = [];
    let videoURL = null;
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        try {
          ...
          console.error('S3 upload failed for file:', file.originalname, uploadErr.message);
          // Continue — save the review without this file rather than crash
        }
      }
    }

    // AUTO-APPROVE Product Reviews, KEEP QR Reviews Pending
    const status = validProductId ? 'approved' : 'pending';
    ...
    if (status === 'approved') {
      await updateProductStats(validProductId);
    }
