...
export default function AgeGroupPage() {
  ...
  if (sortBy === 'Price: Low to High') filtered.sort((a, b) => parseFloat(a.price.replace('$','')) - parseFloat(b.price.replace('$','')));
  if (sortBy === 'Price: High to Low') filtered.sort((a, b) => parseFloat(b.price.replace('$','')) - parseFloat(a.price.replace('$','')));
  if (sortBy === 'Best Rated')         filtered.sort((a, b) => b.stars - a.stars);
  ...
  return (
    <main className="agp-page">
      ...
      {/* 76px spacer to push content below fixed navbar */}
